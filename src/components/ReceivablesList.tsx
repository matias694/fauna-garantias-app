...
export const ReceivablesList: React.FC = () => {
  ...
  // Vencido: Receivables with overdue nextManagementDate or status PENDIENTE
  const totalVencido = receivables.filter(r => r.pendingBalance > 0 && (isOverdue(r.nextManagementDate) || isOverdue(r.createdDate))).reduce((sum, r) => sum + r.pendingBalance, 0);

  // Recuperado este mes: Movements with RECUPERACION_PROPIETARIO / PAGO_ARRENDATARIO / RECUPERACION_FAUNA in current month
  ...
  cases.forEach(c => {
    c.movements.forEach(m => {
      if (m.type === 'PAGO_ARRENDATARIO' || m.type === 'RECUPERACION_FAUNA' || m.type === 'RECUPERACION_PROPIETARIO') {
  ...
  return (
    <div className="p-4 sm:p-6 space-y-5 max-w-7xl mx-auto">
      ...
      {/* Top KPI Cards (3 Cards specified in section 17) */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className="bg-amber-50/70 p-4 rounded-2xl border border-amber-200/80 shadow-2xs">
          ...
          <p className="text-[10px] text-amber-800 font-medium mt-2">Gestiones con atraso de fecha</p>
        </div>
        ...
        <div className="bg-emerald-900 p-4 rounded-2xl text-white shadow-md shadow-emerald-950/10">
          ...
          <p className="text-[10px] text-emerald-200/80 font-medium mt-2">Abonos registrados en agosto 2026</p>
        </div>
      </div>
      ...
      <div className="bg-white p-4 rounded-2xl border border-slate-200/90 shadow-2xs flex flex-col sm:flex-row items-center justify-between gap-3">
        ...
        <span className="text-xs font-bold text-slate-600">
          {filtered.length} cuentas por cobrar
        </span>
      </div>
            ...
            <table className="w-full text-left text-xs">
              ...
              <tbody className="divide-y divide-slate-100 font-medium">
                {filtered.map(r => {
                  ...
                  const responsibleName = correspondingCase?.nextManagementResponsible || correspondingCase?.responsible || 'Encargado Finanzas';
                  ...
                  return (
                    <tr key={r.id} className="hover:bg-slate-50/80 transition-colors">
                      ...
                      {/* Propiedad */}
                      <td className="p-3.5 max-w-xs text-slate-800 font-semibold truncate text-[11px]">
                        {r.propertyAddress}
                      </td>

                      {/* Monto Original */}
                      <td className="p-3.5 text-right font-mono font-bold text-slate-700 text-xs">
                        {formatCLP(r.originalAmount)}
                      </td>

                      {/* Total Pagado */}
                      <td className="p-3.5 text-right font-mono font-bold text-emerald-700 text-xs">
                        {formatCLP(r.totalPaid)}
                      </td>

                      {/* Saldo Pendiente */}
                      <td className="p-3.5 text-right font-mono font-black text-rose-700 text-sm whitespace-nowrap">
                        {formatCLP(r.pendingBalance)}
                      </td>
                      ...
                      {/* Próxima gestión (Acción, Fecha, Responsable) */}
