        ...
        <div className="p-8 space-y-6 overflow-y-auto font-sans text-slate-800 text-xs bg-white print:p-0 print:overflow-visible">
          ...
          <div className="bg-slate-900 text-white p-5 rounded-xl space-y-3 shadow-sm">
            ...
            <div className="border-t border-slate-700 pt-2 flex justify-between items-center text-sm font-bold">
              {fin.isSurplus && (<><span className="text-emerald-400">SALDO A FAVOR DEL ARRENDATARIO A DEVOLVER:</span><span className="font-mono text-emerald-300 text-lg">{formatCLP(fin.refundToTenant)}</span></>)}
              {fin.isExact && (<><span className="text-slate-300">SALDO FINAL LIQUIDACIÓN:</span><span className="font-mono text-white text-lg">$0</span></>)}
              {fin.isInsufficient && (<><span className="text-amber-300">SALDO PENDIENTE A PAGAR POR EL ARRENDATARIO:</span><span className="font-mono text-amber-300 text-lg">{formatCLP(fin.tenantDeficit)}</span></>)}
            </div>
          </div>
        </div>
      </div>
