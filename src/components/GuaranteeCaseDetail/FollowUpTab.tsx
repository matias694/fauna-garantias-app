            ...
            {filteredComments.map(item => {
              ...
              return (
                <div key={item.id} className="bg-slate-50 p-4 rounded-xl border border-slate-200 space-y-2">
                  ...
                  {editing ? (
                    <div className="space-y-3 bg-white border border-slate-200 rounded-xl p-3">
                      <div>
                        ...
                        <select
                          value={editArea}
                          onChange={(e) => setEditArea(e.target.value as FollowUpArea)}
                      ...
                      <div className="flex justify-end gap-2">
                        <button type="button" onClick={cancelEdit} className="px-3 py-1.5 border border-slate-300 rounded-lg text-xs font-bold text-slate-600 inline-flex items-center gap-1 cursor-pointer">
                          <X className="w-3.5 h-3.5" /> Cancelar
                        </button>
                        <button type="button" disabled={!editComment.trim()} onClick={() => saveEdit(item)} className="px-3 py-1.5 bg-slate-900 disabled:bg-slate-300 text-white rounded-lg text-xs font-bold inline-flex items-center gap-1 cursor-pointer">
                  ...
                  {item.editedAt && !editing && (
                    <span className="text-[10px] text-slate-400 block">Última edición: {item.editedAt} · {item.editedBy || 'Usuario'}</span>
                  )}
