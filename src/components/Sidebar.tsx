    ...
    <>
      {isMobileOpen && (
        <button
          ...
          className="fixed inset-0 z-40 bg-slate-950/45 backdrop-blur-[1px] lg:hidden cursor-default"
        />
      )}

      <aside className={`w-64 bg-white text-slate-700 flex flex-col justify-between h-screen border-r border-slate-200/80 shadow-xs select-none z-50 overflow-y-auto transition-transform duration-200 ease-out fixed inset-y-0 left-0 lg:sticky lg:top-0 lg:translate-x-0 ${isMobileOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div>
          <div className="p-5 border-b border-slate-100 flex items-center gap-3">
            ...
            <button
              ...
              onClick={onMobileClose}
              aria-label="Cerrar menú"
              className="lg:hidden p-2 -mr-2 text-slate-400 hover:text-slate-700 hover:bg-slate-100 rounded-xl cursor-pointer"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
