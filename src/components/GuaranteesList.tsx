...
import { Search, Plus, ChevronRight, UserCheck, ShieldAlert, AlertTriangle } from 'lucide-react';
...
export const GuaranteesList: React.FC<GuaranteesListProps> = ({ onOpenNewModal }) => {
  ...
  return (
    <div className="p-4 sm:p-6 space-y-5 max-w-7xl mx-auto">
        ...
        <div className="pt-3 border-t border-slate-100 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-2 text-xs">
          ...
          <div>
            ...
            <select
              ...
              className="w-full bg-slate-50 border border-slate-200 rounded-lg p-1.5 text-xs text-slate-700 font-medium"
            >
      ...
      {/* Simplified Cases Table - Optimized to avoid horizontal scroll */}
              ...
              {filteredCases.length === 0 ? (
                <tr>
                  <td colSpan={7} className="p-8 text-center text-slate-500 text-xs">
                    No se encontraron registros de garantías con los criterios seleccionados.
                  </td>
                </tr>
              ) : (
                filteredCases.map(c => {
                  const fin = calculateGuaranteeFinances(c, settings);
                  const overdueMgmt = c.nextManagementDate && isOverdue(c.nextManagementDate);
                  const formattedDate = formatShortDateStr(c.nextManagementDate);

                  return (
                    <tr 
                      key={c.id} 
                      className={`hover:bg-slate-50/80 transition-colors ${c.isClosed ? 'bg-slate-50/60' : ''}`}
                    >
                      {/* Caso / Propiedad */}
                      <td className="p-3.5">
                        <div className="flex items-center gap-1.5">
                          ...
                          {c.isClosed && (
                            <span className="px-1.5 py-0.2 bg-slate-200 text-slate-700 font-bold text-[9px] rounded uppercase border border-slate-300">
                              Cerrado
                            </span>
                          )}
                        </div>
                        <span className="text-slate-800 font-semibold block text-[11px] mt-0.5">
                          {c.propertyAddress}, {c.propertyUnit}
                        </span>
                      </td>

                      {/* Arrendatario */}
                      <td className="p-3.5">
                        <span className="text-slate-800 font-semibold block text-xs">{c.tenantName}</span>
                      </td>
                      ...
                      {/* Resultado Financiero Humano */}
                      <td className="p-3.5 whitespace-nowrap">
                        {fin.isInsufficient && (
                          <span className="px-2 py-1 rounded-md text-[11px] font-black bg-rose-50 text-rose-800 border border-rose-200 inline-block font-mono">
                            Debe {formatCLP(fin.tenantDeficit)}
                          </span>
                        )}
                        {fin.isSurplus && (
                          <span className="px-2 py-1 rounded-md text-[11px] font-black bg-emerald-50 text-emerald-800 border border-emerald-200 inline-block font-mono">
                            Devolver {formatCLP(fin.refundToTenant)}
                          </span>
                        )}
                        {fin.isExact && (
                          <span className="px-2 py-1 rounded-md text-[11px] font-bold text-slate-500 bg-slate-100 border border-slate-200 inline-block">
                            Sin saldo
                          </span>
                        )}
                      </td>
                      ...
                      <td className="p-3.5 text-right whitespace-nowrap">
                        <button
                          ...
                          className="px-3 py-1.5 bg-[#1E382B] hover:bg-[#14261d] text-white rounded-xl text-xs font-bold inline-flex items-center gap-1 transition-all cursor-pointer shadow-2xs"
                        >
                          <span>Ver caso</span>
